...
class AlbumsFragment : BaseFragment() {
    ...
    private var mSongsAdapter: ItemSearchSongAdapter? = null
    private var mAlbumAdapter: ItemSearchAlbumsAdapter? = null
    private var mPlaylistAdapter: ItemSearchPlaylistAdapter? = null
...
//        FilesUtils.deleteRecursive(File(FilesUtils.getSdcardStore(), Constants.DEFAULT_DATA_FOLDER))
        ...
        isAdvanced = false
        mAdapter = AlbumsAdapter(mContext, ArrayList(), isAdvanced, object : AlbumsAdapter.IOnItemClickListener {
            override fun onItemClick(album: Album, position: Int) {
                ...
                if (playlistDao.getFirstPlaylist().isNotEmpty()) {
                    if (mAdapter.data[position].isPurchase) {
                        (activity!! as MainActivity).addToStackFragment(AlbumDetailGroupFragment.newInstance(album, TAB_ALBUM, -1, false), this@AlbumsFragment)
                    } else {
                    ...
                    Toast.makeText(mContext, mContext!!.getString(R.string.msg_add_first_playlist), Toast.LENGTH_SHORT).show()
                }
            }
        })
        ...
        mSongsAdapter = ItemSearchSongAdapter(mContext!!, mSongs, object : ItemSearchSongAdapter.IOnClickItemListener {
            override fun onClickItem(position: Int) {
                val song = mSongs[position]
                for (album in mAdapter.data) {
                    if (album.id == song.albumId && album.name == song.albumName) {
                        (activity!! as MainActivity).addFragment(AlbumDetailGroupFragment.newInstance(album, TAB_ALBUM, -1, false))//, this@AlbumsFragment
                    }
                }
            }
        })
        mAlbumAdapter = ItemSearchAlbumsAdapter(mContext!!, mAlbumsSearch, object : ItemSearchAlbumsAdapter.IOnClickItemListener {
            override fun onClickItem(position: Int) {
                (activity!! as MainActivity).addFragment(AlbumDetailGroupFragment.newInstance(mAlbumsSearch[position], TAB_ALBUM, -1, false))//, this@AlbumsFragment
            }
        })
        mPlaylistAdapter = ItemSearchPlaylistAdapter(mContext!!, mPlaylist, object : ItemSearchPlaylistAdapter.IOnClickItemListener {
            override fun onClickItem(position: Int) {
                (activity!! as MainActivity).addToStackFragment(AlbumDetailGroupFragment.newInstance(Album(), TAB_ALBUM, mPlaylist[position].id, true), this@AlbumsFragment)
            }
        })
            ...
            0 -> {
                for (i in 0..mAdapter.data.size - 1) {
                    if (mAdapter.data[i].album_priority < 3) {
                        ...
                        mAdapter.data[i].isPurchase = SharedPreferenceHelper.getInstance().getBool(Constants.KEY_PURCHASED)
                    }
                }
            }
            1 -> {
                for (i in 0..mAdapter.data.size - 1) {
                    mAdapter.data[i].isPurchase = SharedPreferenceHelper.getInstance().getBool(Constants.KEY_PURCHASED_ADVANCED)
                }
            }
            2 -> {
                for (i in 0..mAdapter.data.size - 1) {
                    mAdapter.data[i].isPurchase = SharedPreferenceHelper.getInstance().getBool(Constants.KEY_PURCHASED_HIGH_ABUNDANCE)
                }
            }
            3 -> {
                for (i in 0..mAdapter.data.size - 1) {
                    mAdapter.data[i].isPurchase = SharedPreferenceHelper.getInstance().getBool(Constants.KEY_PURCHASED_HIGH_QUANTUM)
                }
            }
        }
        updateView()
    ...
    private fun searchTracks() {
        ...
        val listSongHigherAbundance = database.songDAO().searchTracksByAlbum("%" + edtSearch?.text.toString() + "%", albumHigherAbundanceIds)
        ...
        val listSongHigherQuantum = database.songDAO().searchTracksByAlbum("%" + edtSearch?.text.toString() + "%", albumHigherQuantumIds)
        ...
        //search all album
        val albumSearch = database.albumDAO().searchAlbum("%" + edtSearch?.text.toString() + "%", albumIds)
        mAlbumsSearch.clear()
        for (item in albumSearch) {
            mAlbumsSearch.add(item)
        }
        val albumsAdvanced = database.albumDAO().searchAlbum("%" + edtSearch?.text.toString() + "%", albumAdvancedIds)
        ...
        val albumsHigherAbundanceIds = database.albumDAO().searchAlbum("%" + edtSearch?.text.toString() + "%", albumHigherAbundanceIds)
        ...
        val albumsHigherQuantumIds = database.albumDAO().searchAlbum("%" + edtSearch?.text.toString() + "%", albumHigherQuantumIds)
    ...
    private fun searchPlaylist() {
        ...
        val playlistIds = IntArray(positionPlaylist)
        val playlists = database.playlistDAO().searchPlaylist("%" + edtSearch?.text.toString() + "%")//, playlistIds
        mPlaylist.clear()
