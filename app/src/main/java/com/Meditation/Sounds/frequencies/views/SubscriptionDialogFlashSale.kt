...
import kotlinx.android.synthetic.main.dialog_subscription_flash_sale.*
import java.util.*
import kotlin.collections.ArrayList

class SubscriptionDialogFlashSale(private val mContext: Context?) : Dialog(mContext!!, android.R.style.Theme_Black_NoTitleBar_Fullscreen) {
            ...
            when (mTypeAlbum) {
                0 -> {//Album basic
                    if (random() == 1) {
                        ...
                        mTvUnlockAll.text = TextUtils.concat("Essential Frequencies Annual Subscription")
                        mTvPriceFS.text = SharedPreferenceHelper.getInstance().getPriceByCurrency(Constants.PRICE_1_YEAR_FLASH_SALE)
                        mTvPriceFrom.text = SharedPreferenceHelper.getInstance().getPriceByCurrency(Constants.PRICE_1_YEAR)
                        viewGroupImageAdvanced.visibility = View.GONE
                        viewGroupImageBasic.visibility = View.VISIBLE
                        subs_price.text = context.getString(R.string.tv_yearly_payment_of) + " " + SharedPreferenceHelper.getInstance().getPriceByCurrency(Constants.PRICE_1_YEAR_FLASH_SALE)
                        ...
                        subs_continue.text = SharedPreferenceHelper.getInstance().getPriceByCurrency(Constants.PRICE_1_YEAR_FLASH_SALE) + " / Year"
                        subs_continue.setOnClickListener {
                            baseActivity!!.onPurchaseProduct(Constants.SKU_RIFE_YEARLY_FLASHSALE, false)
                        }
                    } else {
                        //Flash Sale Basic Lifetime
                        mTvUnlockAll.text = TextUtils.concat("Essential Frequencies Lifetime Subscription")
                        mTvPriceFS.text = SharedPreferenceHelper.getInstance().getPriceByCurrency(Constants.PRICE_LIFETIME_FLASH_SALE)
                        mTvPriceFrom.text = SharedPreferenceHelper.getInstance().getPriceByCurrency(Constants.PRICE_LIFETIME)
                        viewGroupImageAdvanced.visibility = View.GONE
                        viewGroupImageBasic.visibility = View.VISIBLE
                        subs_price.text = context.getString(R.string.tv_one_time_payment) + " of " + SharedPreferenceHelper.getInstance().getPriceByCurrency(Constants.PRICE_LIFETIME_FLASH_SALE)
                        subs_info.text = TextUtils.concat(mContext.getString(R.string.tv_title_subscription_new), " ", mSpannableText)
                        subs_info.movementMethod = LinkMovementMethod.getInstance()
                        subs_continue.text = SharedPreferenceHelper.getInstance().getPriceByCurrency(Constants.PRICE_LIFETIME_FLASH_SALE)
                        subs_continue.setOnClickListener {
                            baseActivity!!.onPurchaseProduct(Constants.SKU_RIFE_LIFETIME_FLASHSALE, true)
                        }
                    }
                ...
                1 -> {//Album advanced
                    if (random() == 1) {
                        ...
                        mTvUnlockAll.text = TextUtils.concat("Abundance Frequencies Lifetime Subscription")
                        mTvPriceFS.text = SharedPreferenceHelper.getInstance().getPriceByCurrency(Constants.PRICE_ADVANCED_LIFETIME_FLASH_SALE)
                        mTvPriceFrom.text = SharedPreferenceHelper.getInstance().getPriceByCurrency(Constants.PRICE_ADVANCED_LIFETIME)
                        viewGroupImageAdvanced.visibility = View.VISIBLE
                        viewGroupImageBasic.visibility = View.GONE
                        subs_price.text = context.getString(R.string.tv_one_time_payment) + " of " + SharedPreferenceHelper.getInstance().getPriceByCurrency(Constants.PRICE_ADVANCED_LIFETIME_FLASH_SALE)
                        subs_info.text = TextUtils.concat(mContext.getString(R.string.tv_title_subscription_new), " ", mSpannableText)
                        subs_info.movementMethod = LinkMovementMethod.getInstance()
                        subs_continue.text = SharedPreferenceHelper.getInstance().getPriceByCurrency(Constants.PRICE_ADVANCED_LIFETIME_FLASH_SALE)
                        subs_continue.setOnClickListener {
                            baseActivity!!.onPurchaseProduct(Constants.SKU_RIFE_ADVANCED_LIFETIME_FLASHSALE, true)
                        }
                    } else {
                        //Flash Sale Advanced Yearly
                        mTvUnlockAll.text = TextUtils.concat("Abundance Frequencies Annual Subscription")
                        mTvPriceFS.text = SharedPreferenceHelper.getInstance().getPriceByCurrency(Constants.PRICE_ADVANCED_YEAR_FLASH_SALE)
                        mTvPriceFrom.text = SharedPreferenceHelper.getInstance().getPriceByCurrency(Constants.PRICE_ADVANCED_YEAR)
                        viewGroupImageAdvanced.visibility = View.VISIBLE
                        viewGroupImageBasic.visibility = View.GONE
                        subs_price.text = context.getString(R.string.tv_yearly_payment_of) + " " + SharedPreferenceHelper.getInstance().getPriceByCurrency(Constants.PRICE_ADVANCED_YEAR_FLASH_SALE)
                        subs_info.text = TextUtils.concat(mContext.getString(R.string.tv_title_subscription_new), " ", mSpannableText)
                        subs_info.movementMethod = LinkMovementMethod.getInstance()
                        subs_continue.text = SharedPreferenceHelper.getInstance().getPriceByCurrency(Constants.PRICE_ADVANCED_YEAR_FLASH_SALE) + " / Year"
                        subs_continue.setOnClickListener {
                            baseActivity!!.onPurchaseProduct(Constants.SKU_RIFE_ADVANCED_YEAR_FLASHSALE, false)
                        }
                    }
                ...
                2 -> {//Album Higher
                    ...
                    if (random() == 1) {
                        //Flash Sale  Lifetime
                        mTvPriceFrom.text = SharedPreferenceHelper.getInstance().getPriceByCurrency(Constants.PRICE_HIGHER_LIFETIME)
                        mTvPriceFS.text = SharedPreferenceHelper.getInstance().getPriceByCurrency(Constants.PRICE_HIGHER_LIFETIME_FLASH_SALE)
                        mTvUnlockAll.text = TextUtils.concat("Higher Abundance Frequencies Lifetime Subscription")
                        subs_continue.setOnClickListener {
                            baseActivity!!.onPurchaseProduct(Constants.SKU_RIFE_HIGHER_LIFETIME_FLASH_SALE, true)
                        }
                        subs_price.text = context.getString(R.string.tv_one_time_payment) + " " + SharedPreferenceHelper.getInstance().getPriceByCurrency(Constants.PRICE_HIGHER_LIFETIME_FLASH_SALE)
                    } else {
                        //Flash Sale  Yearly
                        mTvPriceFrom.text = SharedPreferenceHelper.getInstance().getPriceByCurrency(Constants.PRICE_HIGHER_ANNUAL)
                        mTvPriceFS.text = SharedPreferenceHelper.getInstance().getPriceByCurrency(Constants.PRICE_HIGHER_ANNUAL_FLASH_SALE)
                        mTvUnlockAll.text = TextUtils.concat("Higher Abundance Frequencies Annual Subscription")
                        subs_continue.setOnClickListener {
                            baseActivity!!.onPurchaseProduct(Constants.SKU_RIFE_HIGHER_ANNUAL_FLASH_SALE, true)
                        }
                        subs_price.text = context.getString(R.string.tv_yearly_payment_of) + " " + SharedPreferenceHelper.getInstance().getPriceByCurrency(Constants.PRICE_HIGHER_ANNUAL_FLASH_SALE)
                    }
                    subs_info.text = TextUtils.concat(mContext.getString(R.string.tv_title_subscription_new), " ", mSpannableText)
                    subs_info.movementMethod = LinkMovementMethod.getInstance()
                }
